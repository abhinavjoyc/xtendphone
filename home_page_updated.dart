...
class _HomePageState extends ConsumerState<HomePage> with WidgetsBindingObserver {
  ...
  ///
  /// Microphone permission must be granted BEFORE the Android foreground
  /// service is started because this service uses the microphone FGS type.
  Future<void> _onRegistrationChanged(RegistrationState s) async {
    final acc =
    ...
    switch (s) {
      case RegistrationState.registered:
        final permissionsGranted = await _maybeRequestPhonePermissions();

        if (!mounted || !permissionsGranted) return;
  ...
  /// First-successful-registration permission flow.
  ///
  /// The microphone permission is requested first because the Android
  /// foreground service is declared with the "microphone" FGS type.
  /// Notification permission and battery-optimization exemption are handled
  /// afterward as supporting permissions.
  Future<bool> _maybeRequestPhonePermissions() async {
    // Android requires RECORD_AUDIO to be granted before a microphone
    // foreground service can be started.
    final microphoneGranted = await ensureMicrophonePermission();
    if (!microphoneGranted) return false;

    final prefs = ref.read(sharedPreferencesProvider);

    // Microphone permission is checked every time because the user may have
    // denied it previously. The other one-time permission flow only runs once.
    if (prefs.getBool(phonePermissionsAskedKey) ?? false) {
      return true;
    }
